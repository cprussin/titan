...
const strengthResult = (
  ...
  exerciseId: "rdl",
  id: `r-${weightLb}-${reps}`,
  prescription: Prescription.Strength({ reps, sets: 3, weightLb }),
...
describe("progressDouble", () => {
  const base = Prescription.Strength({ reps: 8, sets: 3, weightLb: 185 });
  ...
  it("adds a rep within the range when the target is met below the ceiling", () => {
    ...
    expect(outcome.prescription).toMatchObject({ reps: 9, weightLb: 185 });
  });
  ...
  it("adds weight and resets reps once the top of the range is completed", () => {
    ...
    expect(outcome.prescription).toMatchObject({ reps: 8, weightLb: 190 });
  });
  ...
  it("repeats the ceiling when RPE is over the cap", () => {
    ...
    expect(outcome.prescription).toMatchObject({ reps: 10, weightLb: 185 });
  });
  ...
  it("repeats when the target reps weren't completed", () => {
    ...
    expect(outcome.action).toBe("repeat");
    expect(outcome.prescription).toMatchObject({ reps: 8, weightLb: 185 });
  });
